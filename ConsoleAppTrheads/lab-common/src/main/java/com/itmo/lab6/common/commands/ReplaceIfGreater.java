...
public class ReplaceIfGreater extends BaseCommand {
    ...
    public ReplaceIfGreater(InCollectionManager collectionManager, InDBManager dbManager) {
    ...
    public Response execute(Request request) {
        ...
        try {
            ...
            if (person == null || !person.validate()) throw new WrongArgumentException();
                ...
                Person currentPerson = collectionManager.getCollection().get(personId);
                if (person.compareTo(currentPerson) > 0) {
                    collectionManager.getCollection().replace(personId, currentPerson, person);
                    ...
                    return new Response.ResponseBuilder()
                        .setResponseMessage(String.format("person with id=%d was removed", personId))
                        .setExecutionCode(ExecutionCode.OK)
                        .build();
                }    
                else return new Response.ResponseBuilder()
                    .setResponseMessage(String.format("person with id=%d wasn't removed", personId))
